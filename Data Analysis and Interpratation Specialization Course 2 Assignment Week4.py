...
# # Course 2: Data Analysis Tools: Week 1 ANOVA
...
# # ANOVA
# In my research question, i do not have any quantitative variable, however i have created bins in low medium and high groups. Therefore for the purpose of this Assignment, i am going to use two variables:
# 
# 1. alcohol consumption bins (low, medium, high)
# 2. suicide rate
# 
# Null Hypothesis: There is no significance difference between the mean suicide rate across different levels of alcohol consumption (low, medium, high)
# Alternate Hypothesis: There is a significance difference between the mean suicide rate across different levels of alcohol consumption (low, medium, high)

# In[31]:


df_annova = df_gm[['alcconsumption_bins','suicideper100th']]
df_annova.info()
...
df_annova['alcconsumption_bins'] = df_annova['alcconsumption_bins'].astype('category')
df_annova['suicideper100th'] = pd.to_numeric(df_annova['suicideper100th'], errors = 'coerce')
df_annova.info()
...
# Perform ANOVA (to confirm significance before post hoc)
anova_model = sm.stats.anova_lm(
    sm.OLS.from_formula('suicideper100th ~ C(alcconsumption_bins)', data=df_annova).fit())
print('ANOVA Result:')
print(anova_model)


# ### Summary:
# The p-value is extremely small (7.14e-10), which is much lower than the typical significance level of 0.05. This indicates that there is a statistically significant difference in suicide rates across the different alcohol consumption levels.
# The results of ANOVA test suggest that the mean suicide rates are significantly different across the three alcohol consumption groups. Given the very low p-value, we can reject the null hypothesis and conclude that alcohol consumption levels (low, medium, high) have a statistically significant impact on suicide rates.
...
tukey_result = pairwise_tukeyhsd(
    df_annova['suicideper100th'],
    df_annova['alcconsumption_bins'],
    alpha=0.05
)
...
# Summary:
#     The Tukey HSD post hoc test results indicate that all pairs of alcohol consumption groups (High vs. Low, High vs. Medium, and Low vs. Medium) have statistically significant differences in suicide rates.

# # Course 2: Data Analysis Tools: Week 2 Running a Chi-Square Test of Independence 
...
# Question: Is there an association between alcohol consumption levels and suicide rates across countries?
# Null Hypothesis (H0): The levels of alcohol consumption are independent of the suicide rates.
# Alternative Hypothesis (H1): The levels of alcohol consumption are associated with the suicide rates.
# 
# We already have a categorical variable for alcohol consumption (binned into Low, Medium, High).
# For the Chi-Square test, we will categorize the suicide rates into two levels. Let's create a binary categorical variable based on median split:
# 
#     Low Suicide Rate (below median)
#     High Suicide Rate (above or equal to median)

# In[39]:


# Categorize the suicide rates into low and high based on median
median_suicide_rate = df_annova['suicideper100th'].median()
df_annova['suicide_rate_bins'] = np.where(df_annova['suicideper100th'] < median_suicide_rate, 'Low Suicide Rate', 'High Suicide Rate')
...
contingency_table = pd.crosstab(df_annova['alcconsumption_bins'], df_annova['suicide_rate_bins'])
contingency_table
...
plt.title("Contingency Table: Alcohol Consumption vs. Suicide Rate")
plt.show()


# ### Summary:
# Chi-Square Statistic: 21.4811587578539
# p-value: 2.164839199706632e-05
# Degrees of Freedom: 2
# 
# Given that the p-value is 2.164839199706632e-05 (which is less than 0.05), we reject the null hypothesis and conclude that there is a significant association between alcohol consumption levels and suicide rates. This suggests that alcohol consumption might influence or be related to suicide rates in some way. 

# # Course 2: Data Analysis Tools: Week 3 Generating a Correlation Coefficient 

# Going back to the research question, lets try to  find the correlation uisng suicide rate as the dependent variables and other variables like incomperperon, alcohol consumption, emplyment rate and urbanization and check how the results vary
...
# Calculate the correlation coefficient between employment rate and suicide rate
correlation1 = df_corr['incomeperperson'].corr(df_corr['suicideper100th'])
correlation2 = df_corr['employrate'].corr(df_corr['suicideper100th'])
correlation3 = df_corr['alcconsumption'].corr(df_corr['suicideper100th'])
correlation4 = df_corr['urbanrate'].corr(df_corr['suicideper100th'])

print(f"Correlation coefficient between incomeperperson and suicide rate: {correlation1:.4f}")
print(f"Correlation coefficient between employment rate and suicide rate: {correlation2:.4f}")
print(f"Correlation coefficient between alcohol consumption and suicide rate: {correlation3:.4f}")
print(f"Correlation coefficient between urbanization rate and suicide rate: {correlation4:.4f}")


# ## Summary
# 1. Income per Person: A weak negative correlation suggests that higher income per person might be associated with lower suicide rates, but the relationship is weak.
# 2. Alcohol Consumption: A moderate positive correlation indicates that higher alcohol consumption is associated with higher suicide rates.
# 3. Employment Rate: A moderate negative correlation suggests that higher employment rates are associated with lower suicide rates.
# 4. Urbanization: A weak positive correlation suggests that more urbanized areas might have slightly higher suicide rates.

# # Course 2: Data Analysis Tools: Week 4 Testing a Potential Moderator

# To test for a potential moderator, you'll want to see if the relationship between an independent variable (e.g., employment rate) and a dependent variable (e.g., suicide rate) changes depending on the level of another variable (the moderator). A moderator can be a categorical variable (e.g., alcohol consumption bins) 
# 
# Let's say you want to test whether the relationship between employment rate and suicide rate is moderated by alcohol consumption levels (low, medium, high).
# 
#     Null Hypothesis (H0): The relationship between employment rate and suicide rate does not differ by levels of alcohol consumption.
#     Alternative Hypothesis (H1): The relationship between employment rate and suicide rate differs by levels of alcohol consumption.
...
# Run an NOVA with interaction term (moderation analysis)
# This will assess whether the interaction between employment rate and alcohol consumption affects suicide rate
model = smf.ols('suicideper100th ~ employrate * alcconsumption_bins', data=df_gm).fit()
...
# #### Summary:
# Main Effect of Alcohol Consumption (alcconsumption_bins): F-value: 23.888220 and p-value: 4.628569e-10 (which is extremely small, close to 0)
# The p-value is highly significant, indicating that the level of alcohol consumption (low, medium, high) has a significant effect on suicide rates. This means that suicide rates vary significantly across different levels of alcohol consumption.
#         
# Main Effect of Employment Rate (employrate): F-value: 1.402756 and p-value: 0.2376209
# The p-value is not significant (p > 0.05), suggesting that the employment rate, on its own, does not have a statistically significant effect on suicide rates in this model.
# 
# Interaction Effect (employrate): F-value: 0.596132 and p-value: 0.5518817
# The p-value for the interaction term is not significant (p > 0.05). This suggests that the relationship between employment rate and suicide rate does not significantly vary across different levels of alcohol consumption. In other words, alcohol consumption does not appear to moderate the relationship between employment rate and suicide rate.
# 
# #### Conclusion
# 
# From the analysis, while alcohol consumption has a significant main effect on suicide rates, the employment rate does not significantly affect suicide rates, and there is no significant interaction between employment rate and alcohol consumption. This implies that the impact of employment rate on suicide rates is consistent regardless of the level of alcohol consumption. The null hypothesis for the moderation effect (no interaction) cannot be rejected based on this analysis.
# 
